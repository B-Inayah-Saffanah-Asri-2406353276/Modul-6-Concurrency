...
## Reflection 5
![alt text](/assets/milestone5.png)
Pada Milestone 5, saya mengimplementasikan thread pool untuk mengubah server dari single-threaded menjadi multithreaded agar server dapat diakses bersamaan. Saya membuat struct `ThreadPool` di `src/lib.rs` yang menyimpan sejumlah `Worker`, di mana setiap `Worker` memiliki thread yang berjalan dalam loop menunggu job melalui channel `mpsc`. Untuk berbagi `receiver` antar banyak `Worker`, saya menggunakan `Arc<Mutex<T>>` agar kepemilikan bisa dibagi dan akses ke receiver tetap thread-safe. Ketika `execute` dipanggil, closure dibungkus dalam `Box` sebagai `Job` lalu dikirim melalui channel ke salah satu worker yang idle. Saya juga mengimplementasikan trait `Drop` pada `ThreadPool` untuk graceful shutdown, sender di-drop terlebih dahulu agar semua `recv()` di worker mengembalikan error, sehingga setiap worker keluar dari loop-nya secara bersih sebelum `join` dipanggil, memastikan semua request yang sedang diproses selesai sebelum server benar-benar berhenti.